...
const App = (() => {
    ...
    const _TABLE_COL_CONFIGS = {
        ...
        budgets: {
            ...
            label: 'תקציבים',
            columns: [
                { index: 0, key: 'code', label: 'קוד' },
                { index: 1, key: 'year', label: 'שנה' },
                { index: 2, key: 'period', label: 'תקופה' },
                { index: 3, key: 'description', label: 'תיאור' },
                { index: 4, key: 'amount', label: 'סכום' },
                { index: 5, key: 'freeBalance', label: 'יתרה פנויה' }
            ]
        },
        registrations: {
...
'settings': '⚙️ הגדרות',
    ...
    function _cdRenderFundedStage2() {
        ...
        var hasInternal = allInstructors.some(function(i) { return _getMentorType(i) === 'כוח פנים'; });
        ...
        html += '<div style="margin-bottom:14px;display:flex;flex-direction:column;gap:10px;">' +
            ...
            '<div style="padding:12px 14px;background:var(--gray-50);border:1px solid var(--gray-200);border-radius:var(--border-radius);display:flex;gap:20px;align-items:center;flex-wrap:wrap;">' +
                '<div class="form-group" style="margin-bottom:0;min-width:180px;">' +
                    ...
                    '<div style="padding:8px 12px;background:#fff;border:1px solid var(--gray-200);border-radius:var(--border-radius);font-size:14px;font-weight:500;color:var(--gray-800);">' + escAttr(_dispBTLabel || '—') + '</div>' +
                '</div>' +
                '<div class="form-group" style="margin-bottom:0;min-width:160px;">' +
                    '<label style="font-size:13px;font-weight:600;">סה"כ שעות מתוקצבות</label>' +
                    '<div style="padding:8px 12px;background:#fff;border:1px solid var(--gray-200);border-radius:var(--border-radius);font-size:14px;font-weight:700;text-align:center;color:var(--gray-800);">' + _dispFundedH + '</div>' +
                '</div>' +
            '</div>' +
            // Object 2: Period distribution (read-only) — with date ranges and total relationship
            (function() {
                ...
                return '<div style="padding:12px 14px;background:var(--gray-50);border:1px solid var(--gray-200);border-radius:var(--border-radius);display:flex;gap:12px;align-items:stretch;flex-wrap:wrap;">' +
                    '<div class="form-group" style="margin-bottom:0;flex:1;min-width:130px;">' +
                        ...
                        '<div style="padding:8px 12px;background:#fff;border:1px solid var(--gray-200);border-radius:var(--border-radius);font-size:14px;font-weight:700;text-align:center;color:var(--gray-800);">' + _dispP2 + '</div>' +
                    '</div>' +
                    '<div class="form-group" style="margin-bottom:0;flex:1;min-width:130px;">' +
                        ...
                        '<div style="padding:8px 12px;background:#fff;border:1px solid var(--gray-200);border-radius:var(--border-radius);font-size:14px;font-weight:700;text-align:center;color:var(--gray-800);">' + _dispP1 + '</div>' +
                    '</div>' +
                    ...
                    '<div class="form-group" style="margin-bottom:0;min-width:110px;">' +
                        ...
                        '<div style="padding:8px 12px;background:#fff;border:1px solid var(--gray-200);border-radius:var(--border-radius);font-size:14px;font-weight:700;text-align:center;color:var(--gray-800);">' + _solTotal + '</div>' +
                    '</div>' +
                '</div>' +
        ...
        // Hidden inputs for save/recalc compatibility
        html += '<input type="hidden" id="cd_budgetTypeSelect" value="' + escAttr(s.budgetTypeValue || '') + '">' +
            '<input type="hidden" id="cd_budgetedHours" value="' + (s.budgetedHours || 0) + '">';
    ...
    function _cdRecalc() {
        ...
        rows.forEach(function(sel) {
            ...
            if (isSpecialRecalc) {
                // Special rows now have P1/P2 inputs — read them and validate sum equals fixed total
                ...
                // Show the fixed total in the total cell (read-only, not the sum of inputs)
                ...
                if (type === 'כוח פנים') { internalSum += fixedTotal; intP1 += p1Val; intP2 += p2Val; }
                else { accSum += fixedTotal; accP1 += p1Val; accP2 += p2Val; }
            } else {
        ...
        // Task 4: Update fixed summary row in table footer — show academic periods, note accompaniment if present
        ...
        // Info: Regular vs Funded — budget can be full or partial (3.3)
        ...
        // Check if ALL guides have hours entered in BOTH periods (for conditional validation 3.7)
        ...
        // Conditional validation (3.7): only when ALL guides have hours in both periods
        var mainTotal = regularSum + internalSum;
        if (solTotal > 0 && allHaveHours) {
            var mainDiff = mainTotal - solTotal;
            // Per-period validation
            if (Math.abs(regP1 + intP1 - (regP1 + intP1)) >= 0) { // always true, just for clarity
                // Show per-period breakdown validation
                html += '<div style="background:var(--gray-50);border:1px solid var(--gray-200);border-radius:8px;padding:10px 14px;font-size:12px;color:var(--gray-600);margin-bottom:6px;">ℹ️ <strong>אימות תקופתי:</strong> ' +
                    ...
                    escAttr(p1Label) + ': ' + (regP1 + intP1) + ' שע׳ | סה"כ: ' + mainTotal + ' / ' + solTotal + '</div>';
            }
            if (Math.abs(mainDiff) < 0.01) {
                html += '<div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:10px 14px;color:#166534;font-size:13px;font-weight:500;margin-bottom:6px;">✅ סה"כ שעות אקדמיות (רגיל + כוח פנים): ' + mainTotal + ' = ' + solTotal + ' שע׳ — תואם</div>';
            } else {
                ...
                html += '<div style="background:' + (isOverM ? '#fef2f2' : '#fef9c3') + ';border:1px solid ' + (isOverM ? '#fca5a5' : '#fde047') + ';border-radius:8px;padding:10px 14px;color:' + (isOverM ? '#991b1b' : '#854d0e') + ';font-size:13px;font-weight:500;margin-bottom:6px;">' +
                    ...
                    (isOverM ? 'עודף' : 'חסר') + ' ' + Math.abs(mainDiff) + ' שע׳ מ-' + solTotal + '</div>';
            }
        } else if (anyHoursEntered) {
            // Some guides have hours, some don't — informational only (3.5)
            html += '<div style="background:#f0f9ff;border:1px solid #93c5fd;border-radius:8px;padding:10px 14px;color:#1e40af;font-size:12px;margin-bottom:6px;">ℹ️ לא הוזנו שעות לכל המנחים — ניתן להמשיך בתהליך (3.5). סה"כ שעות שהוזנו: ' + mainTotal + ' / ' + solTotal + '</div>';
        }

        // Note about accompaniment (5.3/3.4)
        if (accSum > 0) {
            html += '<div style="background:#fefce8;border:1px solid #fde68a;border-radius:8px;padding:10px 14px;font-size:12px;color:#713f12;line-height:1.6;margin-bottom:6px;">ℹ️ <strong>הערה:</strong> שעות הליווי (' + accSum + ') הן תקציב נוסף ואינן נכללות בחישוב סה"כ השעות האקדמיות של פתרון הלמידה (' + solTotal + ').</div>';
        }
        ...
        html += '<div style="background:var(--gray-50);border:1px solid var(--gray-200);border-radius:8px;padding:10px 14px;font-size:13px;color:var(--gray-700);display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;">' +
            '<span>📊 סה"כ שעות אקדמיות: <strong>' + mainTotal + '</strong> / ' + solTotal + '</span>' +
    ...
    function _saveCompleteData(solutionId) {
        ...
        if (budgetStatus === 'yes') {
            ...
            // Note: Budget can be full or partial — no blocking validation for regular vs funded (3.1, 3.3)

            // Note: Guide hours are not mandatory — no blocking validation for main vs academic (3.5)
            // Per-period conditional validation is handled visually in _cdRecalc (3.7)
            ...
            // Note: Guide hours are not mandatory — no blocking validation (3.1, 3.5)
            // Visual validation handled in _cdRecalc
            // Update the internal force instructor
            if (nfInstId) {
    ...
    function _renderBudgetsTable(items) {
        ...
        return `<div class="table-wrapper" style="box-shadow:none;"><table class="data-table"><thead><tr><th>קוד</th><th>שנה</th><th>תקופה</th><th>תיאור</th><th>סכום</th><th>יתרה פנויה</th><th>פעולות</th></tr></thead><tbody>
        ${items.map(b => `<tr>
            <td style="direction:ltr"><strong>${b.budgetCode || '—'}</strong></td>
            <td>${b.hebrewYear || '—'}</td><td>${b.period || '—'}</td>
            <td>${b.description || '—'}</td>
            <td>${(parseFloat(b.amount) || 0).toLocaleString('he-IL')} ₪</td>
            <td>${(parseFloat(b.freeBudgetBalance) || 0).toLocaleString('he-IL')} ₪</td>
    ...
    function filterBudgets() {
        ...
        if (search) items = items.filter(b => (b.description||'').toLowerCase().includes(search) || (b.budgetCode||'').toLowerCase().includes(search));
        if (year) items = items.filter(b => b.hebrewYear === year);
        _resetPagination('budgets');
        document.getElementById('budgetsTableDiv').innerHTML = _renderBudgetsTable(items);
        _applyTableFeatures('budgets');
    }
            ...
            <div style="max-height:65vh;overflow-y:auto;padding-right:4px;">
                <div style="margin-bottom:20px;">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group"><label>שנת תקציב (עברית)</label><select id="fBHebYear" class="form-select" onchange="App._onBHebYear()"><option value="">בחר</option>${getHebrewYearOptions(b ? b.hebrewYear : '')}</select></div>
                        <div class="form-group"><label>שנת תקציב</label><input type="text" id="fBEngYear" class="form-input" value="${b ? b.englishYear || '' : ''}" readonly style="background:var(--gray-50);"></div>
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group"><label>תיאור תקציב</label><input type="text" id="fBDesc" class="form-input" value="${b ? b.description || '' : ''}" placeholder="תיאור קצר של התקציב"></div>
                        <div class="form-group"><label>הערה</label><input type="text" id="fBNotes" class="form-input" value="${b ? b.notes || '' : ''}" placeholder="הערות נוספות (אופציונלי)"></div>
                    </div>
                </div>
                <div>
                    <div style="font-size:14px;font-weight:600;color:var(--gray-700);margin-bottom:12px;padding-bottom:8px;border-bottom:2px solid var(--primary);display:flex;align-items:center;gap:8px;">
    ...
    function saveBudget() {
        const data = { budgetCode: document.getElementById('fBCode').value.trim(), hebrewYear: document.getElementById('fBHebYear').value, englishYear: document.getElementById('fBEngYear').value, period: document.getElementById('fBPeriod').value.trim(), estimationStatus: document.getElementById('fBEstStatus').value.trim(), moneyColor: document.getElementById('fBMoneyColor').value.trim(), organizationalUnit: document.getElementById('fBOrg').value.trim(), budgetFor: document.getElementById('fBFor').value, description: document.getElementById('fBDesc').value.trim(), notes: document.getElementById('fBNotes').value.trim(), amount: parseFloat(document.getElementById('fBAmount').value) || 0, planningBalance: parseFloat(document.getElementById('fBPlan').value) || 0, managementBalance: parseFloat(document.getElementById('fBMgmt').value) || 0, freeBudgetBalance: parseFloat(document.getElementById('fBFree').value) || 0 };
